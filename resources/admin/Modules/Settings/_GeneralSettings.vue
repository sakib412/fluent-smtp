    ...
    <div class="fss_general_settings">
        <el-form class="fss_compact_form" :data="settings.misc" label-position="top">
            ...
            <el-form-item label="Log Emails">
                <el-checkbox v-model="settings.misc.log_emails" true-label="yes" false-label="no">
                    {{ $t('Log All Emails for Reporting') }}
                </el-checkbox>
            </el-form-item>

            <el-form-item v-if="settings.misc.log_emails == 'yes' && !!appVars.has_fluentcrm"
                :label="$t('FluentCRM Email Logging')">
                <el-checkbox v-model="settings.misc.disable_fluentcrm_logs" true-label="yes" false-label="no">
                    {{ $t('Disable Logging for FluentCRM Emails') }}</el-checkbox>
            </el-form-item>
            ...
            <el-form-item v-if="settings.misc.log_emails == 'yes'">
                ...
                <el-select v-model="settings.misc.log_saved_interval_days">
                    <el-option v-for="(logLabel, logValue) in logging_days" :key="logValue" :value="logValue"
                        :label="logLabel"></el-option>
                </el-select>
            </el-form-item>
            ...
            <el-form-item>
                ...
                <el-select v-model="settings.misc.default_connection">
                    <el-option v-for="(connection, connectionId) in settings.connections" :key="connectionId"
                        :value="connectionId" :disabled="settings.misc.fallback_connection == connectionId"
                        :label="connection.title + ' - ' + connection.provider_settings.sender_email"></el-option>
                </el-select>
            </el-form-item>
            ...
            <el-form-item>
                ...
                <el-select clearable v-if="connectionsCount > 1" v-model="settings.misc.fallback_connection">
                    <el-option v-for="(connection, connectionId) in settings.connections" :key="connectionId"
                        :disabled="settings.misc.default_connection == connectionId" :value="connectionId"
                        :label="connection.title + ' - ' + connection.provider_settings.sender_email"></el-option>
                </el-select>
                <p v-else style="color: #6d6b6b;margin: 0;">
                    {{ $t('Please add another connection to use fallback feature') }}</p>
            </el-form-item>

            <el-form-item :label="$t('Email Simulation')">
                <el-checkbox v-model="settings.misc.simulate_emails" true-label="yes" false-label="no">
                    {{ $t('Email_Simulation_Label') }}</el-checkbox>
                <p style="color: red;" v-if="settings.misc.simulate_emails == 'yes'">{{ $t('Email_Simulation_Yes') }}</p>
                <p v-if="appVars.is_disabled_defined" style="color: red;">Emails are being simulated due to the definition
                    of <b>FLUENTMAIL_SIMULATE_EMAILS</b> in your PHP code.</p>
            </el-form-item>

            <el-form-item :label="$t('Auto retry')">
